...
from model import Classifier, NPairLoss
...
expt_prefix = "v3"
tlog, vlog = SummaryWriter(
    "../../"+expt_prefix+"logs/train_pytorch"), SummaryWriter("../../"+expt_prefix+"logs/val_pytorch")
...
class TrainValidate():
    ...
    def __init__(self, hiddens=[300, 150, 50], num_epochs=100, initial_lr=1e-3, batch_size=2000, weight_decay=1e-3, load=True):
        ...
        self.net = Classifier(hiddens).to(device)
    ...
    def train_validate(self):
        ...
        sch = torch.optim.lr_scheduler.ReduceLROnPlateau(
            self.optimizer, patience=self.patience, min_lr=1e-7)
        best_loss = np.inf
    ...
    def run_epoch(self, loader, update=True):
        ...
        for batch_index, embedding in enumerate(loader):
           ...
           # print('Embedding size',embedding.size())
            ...
            output_voice_embed, output_face_embed = self.net(
                voice_embedding, face_embedding)  # Net takes voice, faces
    ...
    def load(self, path):
        ...
        self.pload_state_dict(checkpoint['model_state_dict'])
